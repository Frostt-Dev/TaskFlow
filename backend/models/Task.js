  ...
  dueDate: {
    ...
    validate: {
      validator: function(value) {
        // Only validate future date on new document creation
        if (this.isNew) {
...
// Calculate priorityScore on read time (virtual field)
TaskSchema.virtual('priorityScore').get(function() {
  ...
  // daysUntilDue is number of full days rounded down, minimum 1
  const daysUntilDue = Math.max(Math.floor(diffMs / (1000 * 60 * 60 * 24)), 1);
